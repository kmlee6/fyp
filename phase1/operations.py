...
def conv2d(input_tensor, filter_shape, name="conv2d"):
	with tf.variable_scope(name):
		...
		# bias = tf.get_variable('b', filter_shape[-1], initializer=tf.truncated_normal_initializer(stddev=0.02))
...
def fully_connected(input_, input_len, output_len, name="fully_connected"):
	with tf.variable_scope(name):
		...
		# bias = tf.get_variable('b', [output_len], initializer=tf.truncated_normal_initializer(stddev=0.02))
...
def fsconv2d(input_tensor, output_shape, name="fsconv2d"):
	with tf.variable_scope(name):
		...
		# bias = tf.get_variable('b', output_shape[-1], initializer=tf.random_normal_initializer(stddev=0.02))
		output = tf.nn.conv2d_transpose(input_tensor, weights, output_shape=output_shape, strides=[1, 2, 2, 1], padding='SAME')
		output = output + bias
		return output