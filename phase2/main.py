...
if __name__ == "__main__":
	...
	d_opt, g_opt, d_clip, sampler_, monitor, merged = loss(images, image_labels, noise)
	...
	for i in range(check_point+1, step_size):
		...
		for j in range(5):
			...
			sess.run([d_opt, d_clip],{images: image_batch, image_labels: labels, noise: z_batch})
		#train generator
		z_batch = np.random.normal(0, 1, size=[batch_size, 100])
		sess.run([g_opt], {noise: z_batch})
